...
const useBookingForm = ({
  ...
  // event date debounce
  const debouncedEventDate = useDebounce(formData.eventDate, 500);

  useEffect(() => {
    if (debouncedEventDate && !debouncedEventDate.trim()) {
      setErrors((prev) => ({
        ...prev,
        eventDate: "Event date is required",
      }));
    } else if (debouncedEventDate && debouncedEventDate.trim()) {
      setErrors((prev) => ({
        ...prev,
        eventDate: undefined,
      }));
  ...
  const validateForm = useCallback(() => {
    ...
    if (inCludeEventDate && !formData.eventDate) {
    ...
    setErrors(newErrors);
    return isValid;
  }, [formData, includeCheckIn, includeCheckOut, inCludeEventDate]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (inCludeEventDate) {
        descriptionPart.push(`Event Date: ${formData.eventDate}`);
      }
